...
export function CsvImportModal({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => setPreview(parse(ev.target?.result as string))
  ...
  const row = (cells: string[], i: number, dim = false) => (
    <tr key={i} className={dim ? 'bg-gray-50/50 dark:bg-zinc-800/50' : 'bg-white dark:bg-zinc-900'}>
      {columns.map((_, colIndex) => (
        <td key={colIndex} className="px-3 py-2 text-gray-700 dark:text-gray-300">
          {cells[colIndex] ?? ''}
        </td>
      ))}
    </tr>
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-[38rem] bg-white dark:bg-zinc-900 border-gray-200 dark:border-zinc-800">
        <DialogHeader>
                  ...
                  <thead className="bg-gray-50 dark:bg-zinc-800">
                    <tr>
                      {columns.map((h) => (
                        <th
                          key={h}
                          className="px-3 py-2 text-left font-bold uppercase tracking-wider text-gray-600 dark:text-gray-300"
                        >
                          {h}
                        </th>
                      ))}
